...
Server::~Server(){
      ...
      it != connections_.end(); ++it)
  {
    ...
    conn->getLoop()->runInLoop(
      boost::bind(&Connection::connectDestroyed, conn));
    conn.reset();
  }
}
...
void Server::stop(){ //need to ensure
	started_ = false;
...
void Server::newConnection(int sockfd, const InetAddress &peerAddr){
	...
	//char name[32];
	//bzero(name, 32);
	//snprintf(name, 32, "#%d", nextConnId_);
	stringstream nameSS;
	nameSS << '#' << nextConnId_;
	++nextConnId_;
	string name(nameSS.str());
	LOG_DEBUG("Server::newConnection get Loop " << ioLoop << " new Connection name " << name);
	InetAddress localAddr(10086); //empty //FIXME
	ConnectionPtr conn(new Connection(ioLoop, name, sockfd, localAddr, peerAddr));
	
	assert(connections_.find(name) == connections_.end());
	
	conn->setTaskCallback(taskThreadFunc_);
...
void Server::removeConnectionInLoop(const ConnectionPtr& conn){
	...
	LOG_INFO("Server::removeConnectionInLoop");
